import {
  type UserResponse,
  type UserFetchState,
  type UserFetchAction,
} from "types/index";
import { useFetch } from "..";
import { toast } from "react-hot-toast";
import { getCurrentUser } from "@hooks/api";
import { type LoginData } from "types/login";
import { useEffect, useReducer } from "react";
import { userFetchReducer } from "@reducers/user";

interface TimParams {
  filter: {
    is_approved: boolean;
  };
}

export const useGetUser = (filter: any) => {
  const [state, dispatch] = useReducer<
    (state: UserFetchState, action: UserFetchAction) => UserFetchState
  >(userFetchReducer, {
    data: null,
    error: null,
    loading: true,
  });
  const getAllData = useFetch("/user");
  let token: string;
  const user: LoginData = getCurrentUser();
  if (user != null) {
    token = user.access_token;
  }

  const fetchData = async ({ filter }: TimParams) => {
    try {
      const response = await getAllData(
        token,
        `?is_approved=${filter.is_approved}`
      );
      const jsonData: UserResponse = await response.json();
      if (jsonData.meta.status !== 200) {
        return await Promise.reject(jsonData);
      }
      return await Promise.resolve(jsonData);
    } catch (err) {
      return await Promise.reject(err);
    }
  };

  useEffect(() => {
    dispatch({ type: "FETCH_USER_LIST", data: null });
    toast
      .promise<UserResponse>(
        fetchData({ filter }),
        {
          error: "Loading error...",
          success: "Data loaded",
          loading: "Loading...",
        },
        { className: "Inter text-xs" }
      )
      .then((data) => {
        dispatch({ type: "FETCH_USER_LIST_SUCCESS", data });
      })
      .catch((err) => {
        dispatch({ type: "FETCH_USER_LIST_ERROR", data: null });
        console.log(err);
      });
  }, [filter]);
  return {
    loading: state.loading,
    data: state.data,
    error: state.error,
  };
};
